...
COMMAND_FORMAT = """Error! The command should be: py HaarDetection.py --images <images_dir> --masksizeMin <size_in_percentage> --masksizeMax <size_in_percentage> --outputname <name>
                                                    --percisionMin <percision> --percisionMax <percision> --cascade <cascade_dir> --scale <scale> --minNghb <min_neighbour> --json <path_to_json> --plot <not_0_for_plotting_of_images"""
...
def haarDetection(images_dir, output_name, mask_size_min, mask_size_max, percision_min, percision_max, output, cascade_path, scale, min_nghb, json_path, cascade_name, plot_images):
        ...
        if file.endswith(".jpg"):
            ...
            while percision >= percision_min:
                ...
                while mask_size >= mask_size_min:
                    ...
                    constellations, rejectLevels, levelWeights = cascade.detectMultiScale3(gray, float(scale), int(min_nghb), minSize = (200, 200), maxSize = (max_detection, max_detection), flags=cv2.CASCADE_SCALE_IMAGE, outputRejectLevels = True) #minSize = (200, 200), maxSize = (3400, 3400), 
                    
                    print(rejectLevels, levelWeights)
                    ...
                    for (x, y, w, h) in constellations:
                        ...
                        if  len(constellation_stars_data) > int(star_count):
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "h", ["images=", "masksizeMin=", "masksizeMax=", "outputname=", "percisionMin=", "percisionMax=", "cascade=", "scale=", "minNghb=", "json=", "plot="])
    ...
    for file in os.listdir(cascade):
        if file.endswith(".xml"):
            ...
            haarDetection(images_dir, output_name, mask_size_min, mask_size_max, percision_min, percision_max, output, cascade_path, scale, min_nghb, json_path, cascade_name, plot_images)
